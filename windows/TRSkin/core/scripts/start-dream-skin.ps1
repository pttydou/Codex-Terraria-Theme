...
try {
  ...
  if ($savedIsDifferent) {
    ...
    if ($savedProcesses.Count -gt 0 -or $savedOwnsPort) {
      ...
      if ($null -ne $savedIdentity) {
        ...
        Write-Warning 'Reapplying TR Skin to the still-running registered Codex version; the current Store version will be used after that app exits.'
      } else {
        $codexToStop = $savedCodex
  ...
  if (-not $debugReady -and $codexProcesses.Count -gt 0) {
    ...
    if (-not $restartAuthorized -and $PromptRestart) {
      $restartAuthorized = Confirm-DreamSkinRestart -Message 'Codex must restart once to enable TR Skin. Unsaved input may be lost. Restart now?'
      if (-not $restartAuthorized) {
        Write-Host 'TR Skin launch was cancelled; Codex was not changed.'
        exit 0
      }
    }
    if (-not $restartAuthorized) {
      throw 'Codex is open without a verified TR Skin CDP endpoint. Close it first or explicitly use -RestartExisting.'
    }
  ...
  try {
    ...
    if ($verify.ExitCode -ne 0) { throw "TR Skin verification failed. See $VerifyPath" }
  } catch {
    $startupError = $_
  ...
  Write-Host "TR Skin is active on verified loopback port $Port."
} finally {
