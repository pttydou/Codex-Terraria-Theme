...
function Get-DreamSkinCodexProcessCount {
  param([Parameter(Mandatory = $true)][object]$Codex)
  return @(Get-DreamSkinCodexProcesses -Codex $Codex).Count
}
...
function Stop-DreamSkinCodex {
  ...
  while ((Get-DreamSkinCodexProcessCount -Codex $Codex) -gt 0 -and (Get-Date) -lt $deadline) {
  ...
  if ((Get-DreamSkinCodexProcessCount -Codex $Codex) -gt 0) { throw 'Codex could not be stopped safely.' }
}
