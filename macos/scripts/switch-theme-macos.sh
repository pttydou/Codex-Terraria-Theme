...
"$NODE" "$SCRIPT_DIR/random-pool-config.mjs" apply \
  "$RANDOM_POOL_CONFIG_PATH" "$stage/theme.json" >/dev/null \
  || fail "The saved Terraria visual configuration is invalid. Open the skin console and save it again."
...
KEEP_FILES="$($NODE -e '
  ...
  const names = new Set(["theme.json", theme.image, ...Object.values(theme.assets || {})]);
...
for entry in "$THEME_DIR/"*; do
  ...
  if ! /usr/bin/printf '%s\n' "$KEEP_FILES" | /usr/bin/grep -F -x -q "$entry_name"; then
    /bin/rm -f "$entry"
  fi
done
/bin/rm -rf "$stage"
