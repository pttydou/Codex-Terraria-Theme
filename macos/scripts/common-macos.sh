...
RANDOM_POOL_CONFIG_PATH="$STATE_ROOT/random-pool.json"
MUSIC_CONFIG_PATH="$STATE_ROOT/music.json"
MUSIC_LIBRARY_ROOT="$STATE_ROOT/music"
MUSIC_CATALOG_PATH="$PROJECT_ROOT/assets/terraria-music-catalog.json"
...
# Seed bundled music files into the user's music library so a fresh install
# ships with ready-to-play Terraria OST. Idempotent — existing music files are
# preserved; the bundled music.json is only written when the user does not yet
# have one.
seed_bundled_music() {
  local bundled_music="$PROJECT_ROOT/bundled-music"
  local bundled_config="$PROJECT_ROOT/bundled-music.json"
  [ -d "$bundled_music" ] && [ -f "$bundled_config" ] || return 0

  # If the user already has a music.json with tracks, do not overwrite it.
  if [ -f "$MUSIC_CONFIG_PATH" ]; then
    local existing_track_count
    existing_track_count="$(/usr/bin/awk '/"tracks"/{found=1} found&&/":/{count++; if(count>1) exit} END{print count+0}' "$MUSIC_CONFIG_PATH" 2>/dev/null || echo 0)"
    [ "$existing_track_count" -gt 0 ] && return 0
  fi

  /bin/mkdir -p "$MUSIC_LIBRARY_ROOT"
  /bin/chmod 700 "$MUSIC_LIBRARY_ROOT"

  local src dest
  for src in "$bundled_music"/*.mp3; do
    [ -f "$src" ] || continue
    dest="$MUSIC_LIBRARY_ROOT/$(/usr/bin/basename "$src")"
    [ -f "$dest" ] || /bin/cp "$src" "$dest"
    /bin/chmod 600 "$dest" 2>/dev/null || true
  done

  /bin/cp "$bundled_config" "$MUSIC_CONFIG_PATH"
  /bin/chmod 600 "$MUSIC_CONFIG_PATH" 2>/dev/null || true
}
...
seed_bundled_presets() {
  ...
  local presets_root src id dest entry
  for presets_root in "$PROJECT_ROOT/presets" "$PROJECT_ROOT/local-presets"; do
    [ -d "$presets_root" ] || continue
...
# `uname -m` reports the translated process architecture when a script-only
# app is started by LaunchServices under Rosetta. Hardware capability is the
# correct boundary for deciding whether Codex's signed arm64 runtime can run.
macos_hardware_arch() {
  local arm64_capable
...
require_macos_runtime() {
  ...
  local machine_arch
  local node_major
  machine_arch="$(macos_hardware_arch)"
...
write_state() {
  ...
  ' "$STATE_PATH" "$SKIN_VERSION" "$port" "$injector_pid" "$injector_started_at" "$INJECTOR" "$NODE" "$node_ver" "$bundle" "$exe" "$app_ver" "$team" "$PROJECT_ROOT" "$THEME_DIR" "$codex_pid" "$(macos_hardware_arch)" "$session"
}
...
# A manual theme selection authorizes at most one bounded Codex restart. If a
# verified skin port already exists this is a no-op; otherwise the current app
# is closed once and launched once with a loopback-only CDP endpoint. There is
# deliberately no retry loop, watcher, or launchd-owned app process here.
ensure_codex_skin_port_once() {
  local preferred_port="$1"
  case "$preferred_port" in ''|*[!0-9]*) fail "Invalid skin port: $preferred_port" ;; esac
  [ "$preferred_port" -ge 1024 ] && [ "$preferred_port" -le 65535 ] \
    || fail "Skin port must be between 1024 and 65535."

  CODEX_SKIN_PORT="$preferred_port"
  if verified_cdp_endpoint "$CODEX_SKIN_PORT"; then
    export CODEX_SKIN_PORT
    return 0
  fi
  ...
  CODEX_SKIN_PORT="$(select_available_port "$preferred_port")"
  launch_codex_with_cdp "$CODEX_SKIN_PORT"
  if ! wait_for_cdp "$CODEX_SKIN_PORT"; then
    fail "Codex did not expose its local skin port after the single authorized restart. It will not be restarted again."
  fi
  export CODEX_SKIN_PORT
}
